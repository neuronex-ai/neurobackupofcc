...
drop view if exists public.neurofinance_accounts_v;
create view public.neurofinance_accounts_v
with (security_invoker = true)
as
select
  fa.id,
  fa.user_id as professional_id,
  fa.user_id,
  fa.status,
  fa.provider,
  fa.asaas_account_id,
...
drop view if exists public.neurofinance_transactions_v;
create view public.neurofinance_transactions_v
with (security_invoker = true)
as
select
  m.id,
  m.user_id as professional_id,
  m.user_id,
  m.financial_account_id,
  m.provider,
  m.provider_movement_id,
  m.movement_type,
  m.direction,
  m.status,
  m.amount,
...
drop view if exists public.neurofinance_webhook_events_v;
create view public.neurofinance_webhook_events_v
with (security_invoker = true)
as
select
  e.id,
...
grant select on public.neurofinance_accounts_v to authenticated;
grant select on public.neurofinance_transactions_v to authenticated;
grant select on public.neurofinance_webhook_events_v to authenticated;
...
comment on view public.neurofinance_transactions_v is
  'Domain read model for real bank/account movements. Managerial projections must use financial_entries instead.';
