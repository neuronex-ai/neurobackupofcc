...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: document } = await supabase
      ...
      .eq("id", documentId)
      .maybeSingle();
    if (!document) return Response.json({ error: "Documento nao encontrado." }, { status: 404, headers: cors });
    ...
    if (actualSize <= 0 || actualSize > 20 * 1024 * 1024) {
      return Response.json({ error: "Arquivo invalido ou maior que 20 MB." }, { status: 400, headers: cors });
    }
    ...
    console.error("r2-confirm-upload:error", error);
    return Response.json({ error: "Nao foi possivel confirmar o upload." }, { status: 500, headers: cors });
  }
});
