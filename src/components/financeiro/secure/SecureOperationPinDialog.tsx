          ...
          <div className="relative">
            ...
            <div className="mt-6 text-center">
              ...
              <DialogDescription className="mt-2 text-xs leading-relaxed text-zinc-400">
                Digite seu PIN de 6 dígitos para autorizar {actionLabel} de{" "}
                <strong className="text-white">{formatCurrency(value)}</strong> para{" "}
                <strong className="text-white">{recipient || "o destino revisado"}</strong>.
              </DialogDescription>
            </div>
            <div className={cn(
            ...
            <div className="mt-6 grid grid-cols-2 gap-3">
              <Button type="button" variant="ghost" disabled={isLoading} onClick={() => onOpenChange(false)} className="h-12 rounded-[17px] text-[9px] font-black uppercase tracking-widest text-zinc-400 hover:bg-white/5 hover:text-white">
                Voltar
              </Button>
              <Button type="button" disabled={isLoading || pin.length !== 6} onClick={() => onConfirm(pin)} className="h-12 rounded-[17px] bg-white text-[9px] font-black uppercase tracking-widest text-zinc-950 hover:bg-zinc-200">
