...
export const BankAccountsView = () => {
    ...
    const handleSave = async () => {
        ...
        if (!hasBankForm && !hasPixForm) {
            toast.error("Informe uma conta bancária ou uma chave Pix.");
            return;
        }

        if (hasBankForm && (!form.holderName.trim() || digits(form.bankCode).length !== 3 || !digits(form.agency) || !digits(form.account))) {
        ...
        try {
            if (hasBankForm) {
                ...
                if (result?.sync_status === "deferred") {
                    toast.warning("Conta salva no NeuroNex. A sincronização seguirá em segundo plano.");
                }
            }
            ...
            if (hasPixForm) {
                ...
                const { data, error } = await supabase.functions.invoke("neurofinance-post-onboarding", {
                    body: { action: "complete", pix: { key: form.pixKey.trim(), type: "manual" } },
                });
                    ...
                    <div className="relative z-10 space-y-5">
                        <div className="grid gap-4 md:grid-cols-2">
                            ...
                            <SummaryCard icon={KeyRound} title="Chave Pix pessoal" status={hasPixKey ? "Cadastrada" : "Opcional"} active={hasPixKey}>
                                <Info label="Chave Pix" value={storedPixKey || "Não informada"} mono={hasPixKey} />
                            </SummaryCard>
                        </div>

                        {hasStoredAccount && !hasCompleteBankAccount ? (
                            <button type="button" onClick={() => setIsEditing(true)} className="flex w-full items-center gap-3 rounded-2xl border border-amber-500/15 bg-amber-500/[0.06] px-4 py-3 text-left transition-colors hover:bg-amber-500/[0.1] dark:border-amber-300/10">
                                <AlertTriangle className="h-4 w-4 shrink-0 text-amber-600 dark:text-amber-400" />
                                <span className="text-xs font-semibold text-zinc-700 dark:text-zinc-300">A conta {maskedAccount} está registrada. Complete banco e agência para habilitar repasses.</span>
                            </button>
                        ) : null}
                    </div>
                ) : (
...
const SummaryCard = ({ icon: Icon, title, status, active, children }: { icon: typeof Landmark; title: string; status: string; active: boolean; children: React.ReactNode }) => (
    <div className="rounded-[28px] border border-zinc-200/70 bg-zinc-50/70 p-5 dark:border-white/10 dark:bg-white/[0.035]">
        <div className="mb-6 flex items-start justify-between gap-4">
            <div className="flex h-12 w-12 items-center justify-center rounded-2xl border border-black/[0.06] bg-black/[0.03] dark:border-white/[0.08] dark:bg-white/[0.05]"><Icon className="h-5 w-5" /></div>
            <div className="flex items-center gap-2 rounded-full border border-emerald-500/15 bg-emerald-500/[0.06] px-3 py-1.5 text-[9px] font-black uppercase tracking-[0.16em] text-emerald-700 dark:text-emerald-400">
                {active ? <CheckCircle2 className="h-3.5 w-3.5" /> : <LockKeyhole className="h-3.5 w-3.5" />}
                {status}
            </div>
        </div>
        <h4 className="mb-5 text-sm font-black uppercase tracking-[0.18em] text-zinc-950 dark:text-white">{title}</h4>
        {children}
    </div>
);
...
const Field = ({ label, children }: { label: string; children: React.ReactNode }) => (
    <div className="space-y-2">
        ...
        <div className="[&_input]:h-12 [&_input]:rounded-xl [&_input]:border-black/10 [&_input]:bg-white/70 [&_input]:font-bold dark:[&_input]:border-white/10 dark:[&_input]:bg-white/[0.04]">{children}</div>
    </div>
);
