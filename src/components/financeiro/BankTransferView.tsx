...
export const BankTransferView = () => {
  ...
  const manualPixKey = normalizePixKeyInput(pixKey);
  const selectedDestination: RequestPayoutParams["destination"] | null = mode === "saved_bank" ? savedBankDestination : mode === "saved_pix" ? savedPixDestination : { type: "pix_key", pix_key: manualPixKey, summary: pixKey.trim() };
  const canContinue = amountInCents > 0 && Boolean(mode === "saved_bank" ? savedBankDestination : mode === "saved_pix" ? savedPixDestination : pixKey.trim().length >= 5);

  const reset = () => { setStep("setup"); setAmount(""); setPixKey(""); setConsultation(null); setExecution(null); setPinOpen(false); setPinError(null); consult.reset(); authorize.reset(); execute.reset(); };
  ...
  return (
    <div className="mx-auto max-w-4xl py-4">
      <AnimatePresence mode="wait">
        ...
        {step === "review" && consultation && <motion.section key="review" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} className="overflow-hidden rounded-[32px] border border-zinc-200/70 bg-white/75 dark:border-white/[0.08] dark:bg-[#09090b]/80"><div className="border-b border-zinc-100 p-7 dark:border-white/5"><p className="text-[9px] font-black uppercase tracking-[0.25em] text-emerald-600 dark:text-emerald-400">Destino validado</p><h3 className="mt-2 text-xl font-black">Confira antes de sacar</h3><p className="mt-1 text-xs text-zinc-500">{consultation.kind === "payout_pix" ? "A chave foi consultada antes do envio." : "Os dados abaixo são da conta bancária cadastrada no NeuroFinance."}</p></div><div className="p-7"><div className="rounded-[26px] bg-white p-6 text-zinc-950 shadow-2xl"><p className="text-[10px] font-medium uppercase tracking-[0.22em] text-zinc-500">Valor do saque:</p><p className="mt-3 text-5xl font-black tracking-[-0.05em]">{formatCurrency(consultation.amount)}</p></div><div className="mt-6 grid gap-3 md:grid-cols-3">{[{ icon: UserRound, label: "Titular", value: String(destination.holder_name || "Não informado"), detail: maskDocument(String(destination.holder_document || "")) }, { icon: Landmark, label: "Instituição", value: String(destination.bank_name || "Banco cadastrado"), detail: consultation.kind === "payout_pix" ? "Chave Pix validada" : "Conta cadastrada pelo titular" }, { icon: Wallet, label: "Saldo confirmado", value: consultation.availableBalance == null ? "Indisponível" : formatCurrency(consultation.availableBalance), detail: "Consultado ao abrir esta revisão" }].map((item) => <div key={item.label} className="flex items-center gap-4 rounded-[22px] border border-zinc-200/70 bg-zinc-50/70 p-4 dark:border-white/[0.07] dark:bg-white/[0.025]"><div className="flex h-10 w-10 items-center justify-center rounded-[15px] bg-white dark:bg-white/[0.06]"><item.icon className="h-4 w-4" /></div><div className="min-w-0"><p className="text-[8px] font-black uppercase tracking-widest text-zinc-400">{item.label}</p><p className="mt-1 truncate text-sm font-black">{item.value}</p><p className="mt-0.5 text-[10px] text-zinc-500">{item.detail}</p></div></div>)}</div><div className="mt-5 flex items-center gap-2 rounded-[18px] border border-amber-500/15 bg-amber-500/[0.07] px-4 py-3 text-[10px] font-bold text-amber-800 dark:text-amber-200"><ShieldCheck className="h-4 w-4" /> O saque só será enviado depois da confirmação com seu PIN financeiro.</div><div className="mt-6 grid gap-3 sm:grid-cols-[0.75fr_1.25fr]"><Button variant="ghost" onClick={() => { setStep("setup"); setConsultation(null); }} className="h-13 rounded-[18px] text-[10px] font-black uppercase tracking-widest">Corrigir dados</Button><Button onClick={() => { setPinError(null); setPinOpen(true); }} className="h-13 rounded-[18px] bg-zinc-950 text-[10px] font-black uppercase tracking-widest text-white dark:bg-white dark:text-zinc-950"><LockKeyhole className="mr-2 h-4 w-4" /> Confirmar saque</Button></div></div></motion.section>}
...
function Warning({ text }: { text: string }) { return <div className="rounded-2xl border border-amber-500/20 bg-amber-500/10 p-4 text-center text-xs font-semibold text-amber-700 dark:text-amber-300">{text}</div>; }
