...
export function PixTransferir() {
  ...
  const reset = () => {
    setStep("setup"); setPixKey(""); setAmount(""); setDescription(""); setConsultation(null); setExecution(null); setPinOpen(false); setPinError(null);
  ...
  const handleConsult = async () => {
    ...
    try {
      const response = await consult.mutateAsync({ purpose: "transfer", amount: amountInCents, description: description.trim() || undefined, destination: { type: "pix_key", pix_key: normalizedKey, summary: normalizedKey } });
  ...
  return (
    <div className="mx-auto max-w-4xl py-1 sm:py-4">
      <AnimatePresence mode="wait">
        {step === "setup" && (
          <motion.div key="setup" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="mx-auto max-w-2xl space-y-5 sm:space-y-7">
            <section className="rounded-[26px] border border-zinc-200/70 bg-white/70 p-5 dark:border-white/[0.08] dark:bg-white/[0.025] sm:rounded-[30px] sm:p-7">
              ...
              <div className="mt-6 grid grid-cols-2 gap-2 sm:mt-7 sm:grid-cols-5">
                {keyTypes.map((item) => <button key={item.id} type="button" onClick={() => { setKeyType(item.id); setPixKey(""); }} className={cn("h-10 rounded-xl text-[9px] font-black uppercase tracking-wider transition", keyType === item.id ? "bg-zinc-950 text-white dark:bg-white dark:text-zinc-950" : "bg-zinc-100 text-zinc-500 dark:bg-white/5")}>{item.label}</button>)}
              </div>
              <div className="relative mt-4"><KeyRound className="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-zinc-400" /><Input value={pixKey} onChange={(event) => setPixKey(formatPixKeyInput(event.target.value, keyType))} placeholder="Chave Pix do destinatário" className="h-14 rounded-[20px] pl-11 text-sm font-bold dark:border-white/10 dark:bg-white/[0.035]" /></div>
              ...
              <Button onClick={handleConsult} disabled={!pixKey || amountInCents <= 0 || consult.isPending} className="mt-6 h-14 w-full rounded-[20px] bg-zinc-950 text-[10px] font-black uppercase tracking-[0.18em] text-white dark:bg-white dark:text-zinc-950 sm:h-15 sm:rounded-[22px]">
        ...
        {step === "review" && consultation && (
          <motion.section key="review" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} className="overflow-hidden rounded-[28px] border border-zinc-200/70 bg-white/75 dark:border-white/[0.08] dark:bg-[#09090b]/80 sm:rounded-[32px]">
            <div className="border-b border-zinc-100 p-5 dark:border-white/5 sm:p-7"><p className="text-[9px] font-black uppercase tracking-[0.25em] text-emerald-600 dark:text-emerald-400">Destino Pix validado</p><h3 className="mt-2 text-xl font-black">Confira antes de transferir</h3><p className="mt-1 text-xs text-zinc-500">A chave foi consultada diretamente no DICT pela Asaas.</p></div>
            <div className="p-5 sm:p-7">
              <div className="rounded-[24px] bg-white p-5 text-zinc-950 shadow-2xl sm:rounded-[26px] sm:p-6"><p className="text-[10px] font-medium uppercase tracking-[0.22em] text-zinc-500">Valor da transferência:</p><p className="mt-3 text-3xl font-black tracking-[-0.05em] sm:text-5xl">{formatCurrency(consultation.amount)}</p></div>
              ...
              <div className="mt-5 flex items-center gap-2 rounded-[18px] border border-amber-500/15 bg-amber-500/[0.07] px-4 py-3 text-[10px] font-bold text-amber-800 dark:text-amber-200"><ShieldCheck className="h-4 w-4 shrink-0" /> A transferência só será enviada depois da confirmação com seu PIN financeiro.</div>
