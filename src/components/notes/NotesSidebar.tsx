...
export const NotesSidebar = ({
  ...
  const renderItem = (item: { id: string; label: string; icon: any; mode: string }) => {
    ...
    return (
      <button
        ...
        className={cn(
          "w-full flex items-center justify-between group px-3 py-2 rounded-xl transition-all duration-500 relative overflow-hidden",
          isActive
      ...
      >
        <div className="flex items-center gap-3 relative z-10">
          <item.icon className={cn("h-3.5 w-3.5 shrink-0 transition-all duration-500", isActive ? "scale-110" : "opacity-40 group-hover:opacity-100")} strokeWidth={isActive ? 2.5 : 1.5} />
          <span className="text-[9px] font-black uppercase tracking-[0.22em] whitespace-nowrap">{item.label}</span>
        </div>
        {isActive && (
          <motion.div
  ...
  return (
    <div className="flex flex-col h-full w-full bg-transparent font-sans relative">
      ...
      {/* Brand Header */}
      <div className="px-6 pt-8 pb-5 relative z-10">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h2 className="text-[8px] font-black text-zinc-400 dark:text-zinc-500 uppercase tracking-[0.48em]">NeuroDrive</h2>
        </div>
      ...
      <div className="flex-1 overflow-y-auto px-3 space-y-6 pb-8 custom-scrollbar relative z-10">

        {/* New Note Button with favicon */}
        <div className="px-2">
          <button
            onClick={onCreateNote}
            className="w-full flex items-center gap-3 px-3.5 py-2.5 rounded-2xl bg-zinc-900 dark:bg-white text-white dark:text-black shadow-sm hover:shadow-md transition-all duration-300 hover:scale-[1.015] active:scale-[0.985] group"
          >
            ...
            />
            <span className="text-[9px] font-black uppercase tracking-[0.2em]">Nova Nota</span>
        ...
        <div className="space-y-2">
          <h3 className="px-3 text-[7.5px] font-black uppercase tracking-[0.34em] text-zinc-400 dark:text-zinc-600 mb-3">Sincronia</h3>
          ...
          <div className="mt-6 space-y-3">
            <div className="flex items-center justify-between px-3 group/folder-header">
              <span className="text-[7.5px] font-black uppercase tracking-[0.34em] text-zinc-400 dark:text-zinc-600">Modulos</span>
              <button
              ...
              {modules?.map((module) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "w-full flex items-center justify-between group px-3 py-2 rounded-xl transition-all duration-500 border select-none",
                      isActive
                  ...
                  >
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                      ...
                      <span className="text-[9px] font-bold tracking-tight truncate uppercase">{module.name}</span>
                    </div>
                    <button
                      onClick={(e) => { e.stopPropagation(); if (confirm("Excluir este modulo?")) deleteModule(module.id); }}
                      className={cn(
        ...
        {/* ─── REDES NEURAIS ─── */}
        <div className="space-y-2">
          <h3 className="px-3 text-[7.5px] font-black uppercase tracking-[0.34em] text-zinc-400 dark:text-zinc-600 mb-3">Inteligencia</h3>
          <div className="space-y-1">
            {[
