          ...
          >
            <section className="grid grid-cols-2 gap-3">
              {[
                ...
                <div key={label} className="rounded-[26px] border border-border/40 bg-card/75 p-4 dark:border-white/10 dark:bg-white/[0.03]">
                  <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-foreground/[0.05] text-muted-foreground">
                    <Icon className="h-4 w-4" />
                  </div>
                  <p className="mt-4 text-[8px] font-black uppercase tracking-[0.17em] text-muted-foreground/60">{label}</p>
                  <p className="mt-1 text-lg font-black tracking-[-0.04em] text-foreground">
                    {isLoadingMetrics ? "—" : formatCurrency(value)}
                  </p>
                  <p className="mt-1 text-[9px] font-medium leading-relaxed text-muted-foreground/55">{helper}</p>
                </div>
              ))}
            </section>
            ...
            <section ref={cashflowRef} className="scroll-mt-28 space-y-3">
              <MobileSectionHeader
                eyebrow="Últimos lançamentos"
                title="Fluxo recente"
                description="Movimentação gerencial registrada no consultório."
              />
              <div className="rounded-[28px] border border-border/40 bg-card/75 p-5 dark:border-white/10 dark:bg-white/[0.03]">
                  ...
                  <div className="h-36 w-full">
                    <ResponsiveContainer width="100%" height="100%">
                      <AreaChart data={chartData}>
                        ...
                        <Area type="monotone" dataKey="value" stroke="currentColor" strokeWidth={2} fill="url(#managementCashflow)" />
                      </AreaChart>
                    </ResponsiveContainer>
                  </div>
                ) : (
            ...
            <section ref={transactionsRef} className="scroll-mt-28 space-y-4">
              <MobileSectionHeader
              ...
              <div className="flex gap-2 overflow-x-auto pb-1 no-scrollbar">
                {[
                  ...
                  <button
                    ...
                    className={cn(
                      "min-h-10 shrink-0 rounded-xl px-4 text-[8px] font-black uppercase tracking-[0.13em] transition-all",
              ...
              {isLoadingTransactions ? (
                ...
                <div className="space-y-2">
                  {filteredTransactions.slice(0, 12).map((transaction) => (
                    <div
                      key={transaction.id}
                      className="flex items-center gap-3 rounded-[22px] border border-border/40 bg-card/75 p-4 dark:border-white/10 dark:bg-white/[0.03]"
                    >
                      <div className={cn(
            ...
            >
              {isLoadingAccount ? (
                ...
                <section className="overflow-hidden rounded-[32px] border border-border/40 bg-foreground p-6 text-background shadow-xl dark:border-white/10">
                  <div className="flex h-14 w-14 items-center justify-center rounded-[20px] bg-background/10">
                  ...
                  <Button
                    ...
                    className="mt-7 h-13 w-full rounded-2xl bg-background text-[9px] font-black uppercase tracking-[0.17em] text-foreground hover:bg-background/90"
                  >
                    Começar agora <ArrowRight className="ml-2 h-4 w-4" />
                  </Button>
                  <div className="mt-6 opacity-60"><AsaasRegulatoryFooter /></div>
                </section>
              ) : (
                <div className="space-y-6">
                  ...
                  <section className="relative overflow-hidden rounded-[30px] border border-border/40 bg-card/80 p-6 dark:border-white/10 dark:bg-white/[0.035]">
                    <div className="absolute -right-20 -top-20 h-44 w-44 rounded-full bg-foreground/[0.05] blur-3xl" />
                    <div className="relative flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <div className="flex h-11 w-11 items-center justify-center rounded-[17px] bg-foreground/[0.05] text-muted-foreground">
                          <Wallet className="h-5 w-5" />
                        </div>
                        <div>
                          <p className="text-[8px] font-black uppercase tracking-[0.18em] text-muted-foreground/55">Saldo disponível</p>
                          <p className="mt-1 text-[10px] font-bold text-muted-foreground">Conta NeuroFinance</p>
                        </div>
                      </div>
                      <button
                        type="button"
                        onClick={() => setShowBalance((current) => !current)}
                        className="flex h-10 w-10 items-center justify-center rounded-2xl bg-foreground/[0.045] text-muted-foreground"
                      >
