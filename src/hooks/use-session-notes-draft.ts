...
const readDraft = async (key: string): Promise<SessionNotesDraftRecord | null> => {
  try {
    ...
    // The localStorage fallback below keeps recovery available on restricted browsers.
  }
...
export const useSessionNotesDraft = (appointmentId: string) => {
  const { user } = useAuth();
  ...
  useEffect(() => {
    if (restoredKeyRef.current !== key || syncState === 'restoring') return;
    setSyncState('saving');
