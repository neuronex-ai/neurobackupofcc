...
export function useR2Documents(patientId?: string | null) {
  ...
  const upload = useMutation({
    mutationFn: async ({ file, patientId: targetPatientId, category = "general", metadata = {} }: UploadInput) => {
      ...
      const { data: document, error: prepareError } = await supabase.rpc("prepare_document_upload", {
      ...
      if (prepareError || !document) throw prepareError ?? new Error("Não foi possível registrar o documento.");
      ...
      if (!uploadResponse.ok) {
        await supabase.from("document_files").update({ status: "failed" }).eq("id", document.id);
        throw new Error("O armazenamento recusou o arquivo. Verifique o CORS do bucket R2.");
      }
